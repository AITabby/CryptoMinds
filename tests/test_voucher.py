...
class TestVoucherAPI:
    ...
    def test_voucher_limit_preview(self, client):
        """测试额度预览"""
        resp = client.post(
    ...
    def test_voucher_create(self, client):
        """测试创建 Voucher"""
        resp = client.post(
    ...
    def test_voucher_get(self, client):
        """测试查询 Voucher"""
        # 先创建
    ...
    def test_voucher_use(self, client):
        """测试使用 Voucher"""
        # 先创建
        create_resp = client.post(
            "/api/v1/voucher/create",
    ...
    def test_voucher_exhaust(self, client):
        """测试 Voucher 耗尽"""
        # 创建 10 单位的 voucher
        create_resp = client.post(
            "/api/v1/voucher/create",
    ...
    def test_voucher_overuse(self, client):
        """测试超额使用"""
        # 创建 10 单位的 voucher
        create_resp = client.post(
            "/api/v1/voucher/create",
    ...
    def test_voucher_list(self, client):
        """测试列表查询"""
        # 创建几个 voucher
        for i in range(3):
            client.post(
    ...
    def test_voucher_list_by_agent(self, client):
        """测试按 Agent 查询"""
...
class TestVoucherStore:
    ...
    def test_create_voucher(self, store):
        """测试存储创建"""
        import uuid
    ...
    def test_get_voucher(self, store):
        """测试存储查询"""
        import uuid
        voucher_id = f"test_voucher_{uuid.uuid4().hex[:8]}"
    ...
    def test_use_voucher(self, store):
        """测试存储使用"""
        import uuid
        voucher_id = f"test_voucher_{uuid.uuid4().hex[:8]}"
        store.create_voucher(
        ...
        result = store.use_voucher(voucher_id, 30)
        assert result["units_used"] == 30
        assert result["status"] == "issued"

    def test_list_vouchers(self, store):
        """测试存储列表"""
        vouchers = store.list_vouchers(agent_id="agent_test")
