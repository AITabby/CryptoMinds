...
async function agentBuyToken() {
  ...
  if (btn) { btn.disabled = true; btn.classList.add('opacity-disabled'); btn.innerHTML = '<span class="spin-icon">⏳</span> 执行中...'; }
  try {
  ...
  _buyingActive = false;
  if (btn) { btn.disabled = false; btn.classList.remove('opacity-disabled'); }
}
...
function renderBuyerTxTable(orders, force) {
  ...
  if (orders.length === 0) {
    if (noTx) noTx.classList.remove('hidden');
    return;
  }
  if (noTx) noTx.classList.add('hidden');
  const sorted = [...orders].sort((a, b) => new Date(b.time || 0) - new Date(a.time || 0));
  sorted.forEach((o, idx) => {
    if (o.rated) return;
    const time = new Date(o.time).toLocaleString('zh-CN', {timeZone:'Asia/Shanghai', hour12:false});
