...
const AGENT_CSS = { 'ChainSentry': 'text-up', 'RiskGuard': 'text-primary', 'NFTScout': 'text-down', 'GasSaver': 'text-info', 'AlphaBot': 'text-primary', 'ChainSeer': 'text-primary', 'Sentinel': 'text-up', 'Buyer Agent': 'text-primary', 'Scout Agent': 'text-info', 'Momentum One': 'text-up', 'Dip Hunter': 'text-primary', 'Risk Sentinel': 'text-info', 'Flow Surfer': 'text-up' };
...
function renderTxEvent(tx) {
  ...
  let agent, icon, cssClass, reason, direction;
  if (isMyTo && !isMyFrom) {
    agent = tx.to;
    icon = AGENT_ICONS[agent] || '🤖';
    cssClass = AGENT_CSS[agent] || 'text-primary';
  ...
  html += '<span>' + icon + '</span>';
  html += '<span class="live-event-agent ' + cssClass + '">' + agent + '</span>';
  if (direction) html += '<span class="text-muted">' + direction + '</span>';
  html += '<span class="live-event-time">' + formatTime(tx.timestamp) + '</span>';
  html += '</div>';
  html += '<div class="live-event-body">';
  if (isMyTo && !isMyFrom) {
    html += direction + ' ' + reason;
    if (tx.from) html += ' ← <strong class="text-body">' + tx.from + '</strong>';
  } else {
    html += reason;
    if (tx.to && tx.to !== agent) html += ' → <strong class="text-body">' + tx.to + '</strong>';
  }
  html += ' <span class="text-up">' + tx.amount + ' BNB</span>';
  html += '</div>';
  if (shortTx) {
    html += '<div class="live-event-detail">';
    if (bscUrl) {
      html += '<a class="live-event-tx" href="' + bscUrl + '" target="_blank">🔗 ' + shortTx + '</a>';
      html += '<span class="text-up">✅ 链上验证</span>';
    } else {
      html += '<span class="text-muted font-number">' + shortTx + '</span>';
      if (tx.verified) html += '<span class="text-muted-strong">' + tx.verified + '</span>';
    }
    if (tx.route_type) html += '<span class="text-muted">' + tx.route_type + '</span>';
    html += '</div>';
  }
  html += '</div>';
  return html;
...
function renderEventItem(item) {
  if (item._type === 'event') {
    ...
    const cssClass = AGENT_CSS[item.agent] || 'text-primary';
    ...
    html += '<div class="live-event-header">';
    html += '<span>' + icon + '</span>';
    html += '<span class="live-event-agent ' + cssClass + '">' + item.agent + '</span>';
    html += '<span class="text-muted">' + (typeLabel[item.type] || item.type) + '</span>';
...
async function loadLiveFeed() {
  ...
  try {
    ...
    if (myAgentEl) myAgentEl.innerHTML = '<div class="text-down text-center" style="padding:40px 0;">加载失败: ' + e.message + '</div>';
    if (marketEl) marketEl.innerHTML = '<div class="text-down text-center" style="padding:40px 0;">加载失败: ' + e.message + '</div>';
  }
}
...
function renderFeed(containerId, items, showChain) {
  ...
  if (!el) return;
  if (!items.length) {
    el.innerHTML = '<div class="text-muted text-center" style="padding:40px 0;">暂无活动</div>';
    return;
  }
  let html = '';
  if (showChain) {
...
async function startLiveStream() {
  ...
  if (statusEl) { statusEl.textContent = '● 连接中...'; statusEl.className = 'text-primary'; }
  ...
  liveEventSource.onmessage = function(e) {
    try {
      ...
      if (item._type === 'connected') {
        if (statusEl) { statusEl.textContent = '● 实时'; statusEl.className = 'text-up'; }
        if (pulseEl) pulseEl.classList.add('live-pulse');
        return;
      }
  ...
  liveEventSource.onerror = function() {
    if (statusEl) { statusEl.textContent = '○ 重连中...'; statusEl.className = 'text-down'; }
    if (pulseEl) { pulseEl.classList.add('pulse-trading-down'); }
...
function prependLiveEvent(item) {
  ...
  setTimeout(() => { el.classList.remove('live-flash'); }, 1500);
  ...
  if (elLastUpdate) elLastUpdate.textContent = '更新于 ' + now.toLocaleTimeString('zh-CN', { hour: '2-digit', minute: '2-digit', second: '2digit' });
  App.refreshLucide();
}
...
function renderTxsRow(tx) {
  ...
  const toW = (tx.to_wallet || tx.toWallet || '').toLowerCase();
  const fromW = (tx.from_wallet || tx.fromWallet || '').toLowerCase();
  const isIn = wallet && toW && toW === wallet;
  const icon = isIn ? 'download' : 'upload';
  const iconCss = isIn ? 'text-up' : 'text-primary';
  const sign = isIn ? '+' : '-';
  const valClass = isIn ? 'pos' : 'neg';

  const fromName = tx.from_name || tx.from || '?';
  const toName = tx.to_name || tx.to || '?';

  let html = '<div class="tx-item">';
  html += '<div class="tx-icon ' + (isIn ? 'in' : 'out') + '"><i data-lucide="' + icon + '" class="icon-sm ' + iconCss + '"></i></div>';
  html += '<div class="tx-info">';
  html += '<div class="flow">' + fromName + ' → ' + toName + '</div>';
  html += '<div class="reason">' + (tx.reason || '') + (bscUrl ? ' <a href="' + bscUrl + '" target="_blank" class="link-primary">🔗</a>' : '') + '</div>';
  html += '</div>';
...
async function loadTxsFeed() {
  try {
    ...
    if (!txs.length) {
      el.innerHTML = '<div class="text-muted text-center" style="padding:30px 0;">暂无交易</div>';
      return;
    }
    ...
    const el = document.getElementById('txList');
    if (el) el.innerHTML = '<div class="text-muted text-center" style="padding:30px 0;">暂无交易</div>';
  }
}
